...
Prior (per Firmenich et al. 2026, §3.3 + Tab. prior_sensitivity):
    Uniform[1.5, 3.0] (default)
    Lower bound 1.5 accommodates near-planar structures;
    upper bound 3.0 is the embedding dimension.
    Robustness verified under Beta(7.5,2.5), Beta(2.5,7.5), Beta(5,5)
    all rescaled to [1.5, 3.0]; sigma_c varies <0.6 km.
...
- Effective sample size (ESS > 5,000)
- Autocorrelation time (τ < n_steps/50)

Saturation Indicators:
----------------------
1. Posterior mass at D₃ ∈ [2.95, 3.00]
...
Date: 2025-12-17 (updated 2026-04-08 to align with v6 manuscript)
...
# ── Prior parameters (per Firmenich et al. 2026, §3.3+Tab.prior_sensitivity) ──
# Default: Uniform[1.5, 3.0] per main manuscript.
# Lower bound 1.5 accommodates near-planar structures.
# Upper bound 3.0 is the embedding dimension.
# Robustness verified: Beta(7.5,2.5), Beta(2.5,7.5), Beta(5,5) all rescaled to
# [1.5,3.0] produce sigma_c variations <0.6 km (Tab. prior_sensitivity).
PRIOR_LOWER: float = 1.5  # D3 lower bound
PRIOR_UPPER: float = 3.0  # D3 upper bound (Euclidean embedding)

# Fisher Information Barrier (Firmenich et al., 2026)
# Threshold where Bayesian inference begins to saturate (D₃ -> 3.00)
FISHER_BARRIER_SIGMA: float = 2.3  # km

# MCMC defaults (per §3.5, emcee configuration validated for ESS>5,000)
MCMC_N_WALKERS: int = 32
MCMC_N_STEPS: int = 10_000   # per Sysmic v8.0.0 manuscript campaign
MCMC_N_BURNIN: int = 2_000   # per Sysmic v8.0.0 manuscript campaign


def log_prior_d3(
    d3: float,
    lower: float = PRIOR_LOWER,
    upper: float = PRIOR_UPPER,
) -> float:
    """
    Log-prior for D₃: Uniform[PRIOR_LOWER, PRIOR_UPPER].

    Default: Uniform[1.5, 3.0] per Firmenich et al. (2026) §3.3.
    Fisher prior regularization: I_prior = 1/(upper-lower)² = 0.444.

    Args:
        d3: Fractal dimension value
        lower: Support lower bound (default 1.5)
        upper: Support upper bound (default 3.0)

    Returns:
        Log-probability (0.0 if in support, -inf otherwise)
    """
    if lower <= d3 <= upper:
        return -np.log(upper - lower)  # log(1/1.5) = -0.405
    return -np.inf


# ── Keep backward-compatible alias with alpha/beta signature ──────────────────
_log_prior_d3_legacy = log_prior_d3
...
def bayesian_d3_inference_emcee(
    ...
    **likelihood_kwargs
    ...
    Per Sysmic v8.0.0 manuscript campaign (Firmenich et al. 2026):
    32 walkers × 10,000 steps, 2,000 burn-in, R̂<1.01, ESS>5,000.
    ...
    Returns:
        Dictionary with:
            ...
            - posterior_mass_saturation: % mass at D₃∈[2.95,3.00]
            - gelman_rubin: R̂ convergence diagnostic
            - acceptance_fraction: Mean acceptance rate
            - n_effective_samples: Post burn-in sample count
    """
    if not HAS_EMCEE:
    ...
    # Saturation indicator: posterior mass at D₃ ∈ [2.95, 3.00]
    saturation_mask = (samples_flat >= 2.95) & (samples_flat <= 3.00)
    ...
    if verbose:
        ...
        print(f"  Posterior mass @ [2.95,3.00]: {posterior_mass_saturation:.1f}%")
...
def bayesian_d3_inference_fallback(
    ...
    **likelihood_kwargs
    ...
    saturation_mask = (samples >= 2.95) & (samples <= 3.00)
...
def compute_kl_divergence_d3(
    ...
    conc_pct = 100 * np.sum((posterior_samples >= 2.95) & (posterior_samples <= 3.00)) / len(posterior_samples)
    
    if verbose:
        print(f"  KL divergence: {kl_divergence:.3f} nats")
        print(f"  Posterior conc. @ D₃∈[2.95,3.00]: {conc_pct:.1f}%")
